...
//	Experimental variables
	float DDM_adj <- 0.01;
	float tx_competitiveness <- 0.008;
	float release_ratio <- 25.0;
	int release_period <- 3;
	int release_delay <- 365;
	float InCuBUS_lethality <- 1.00;
	float InCuBUS_reduced_lethality <- 0.55;
...
//	transgenic parameters
	float fem_fsRIDL_surv <- 0.965;
	bool exp_InCuBUSfsRIDL <- false;
	bool exp_fsRIDL <- false;
	bool exp_InCuBUS_reduced <- false;
	bool exp_InCuBUS <- false;
	bool exp_WT <- false;
	bool exp_Tx function: 	exp_InCuBUSfsRIDL or
							exp_InCuBUS_reduced or
							exp_fsRIDL or 
							exp_InCuBUS? true: false;
...
//	Release calculations
	...
	reflex release when: exp_Tx and
						 new_day and
...
//	Time to 50% female population reduction
	reflex when: nb_days >= release_delay and p50 = 0 and length(female_mosquito) <= nb_females_init * 0.5 {
		...
		to: "InCuBUS_PR50.csv" format: csv rewrite: false;
	}
...
//	Time to 95% female population reduction
	reflex when: nb_days >= release_delay and p95 = 0 and length(female_mosquito) <= nb_females_init * 0.05 {
		...
		to: "InCuBUS_PR95.csv" format: csv rewrite: false;
	}
	...
	reflex step_save when: is_batch and nb_days >= release_delay  {
		...
		to: "InCuBUS_step_output.csv" format: csv rewrite: false;
	}
...
species larvae_L3 {
	...
	reflex age {
		...
		if (cycle >= age + hours_pupate) {
			...
			if (female and fsRIDL_carrier and flip(fem_fsRIDL_surv)) {
				do die;
			}
			else {
				create (pupae) {
...
//		If she's already gone through a gonotrophic cycle, isn't currently gravid,
// 		... and enough time as past, she will get hungry
...
//		Decide activity based on time of day
	...
	action death {
		...
		if (nb_days >= release_delay and is_batch) {
			...
			to: "InCuBUS_female_output.csv" format: csv rewrite: false;
		}
		do die;
	}
...
//					Re-mating with a fsRIDL male displaces fsRIDL carrier sperm
				...
				if (myself.mated and InCuBUS) {
					if (flip(InCuBUS_lethality)) {
						myself.toxin <- true;	
					}
				}
				if (myself.mated and InCuBUSfsRIDL) {
					myself.fsRIDL <- true;
//					Re-mating with an IncuBUSfsRIDL male displaces fsRIDL carrier sperm
					if (myself.fsRIDL_carrier) {
						myself.fsRIDL_carrier <- false;
					}
					if (flip(InCuBUS_lethality)) {
...
species transgenic_mosquito parent: male_mosquito {
	bool InCuBUS <- false;
	bool InCuBUSfsRIDL <- false;

	init {
		if (exp_fsRIDL = true) {
			self.fsRIDL <- true;
		}
		else if (exp_InCuBUS = true) {
			self.InCuBUS <- true;
		}
		else if (exp_InCuBUSfsRIDL = true) {
			self.InCuBUSfsRIDL <- true;
		}
		else if (exp_InCuBUS_reduced = true) {
			self.InCuBUS <- true;
		}
	}
}

experiment WT type: batch repeat: 10 keep_seed: false until: nb_days >= (release_delay + 120) {
...
experiment InCuBUS type: batch repeat: 10 keep_seed: false until: nb_days >= (release_delay + 120) {
	parameter "InCuBUS" var:exp_InCuBUS init: true;
	parameter "Batch" var:is_batch init: true;
...
experiment InCuBUS_reduced type: batch repeat: 10 keep_seed: false until: nb_days >= (release_delay + 120) {
	parameter "InCuBUS_reduced" var:exp_InCuBUS_reduced init: true;
	parameter "Batch" var:is_batch init: true;
	parameter "Lethality" var:InCuBUS_lethality init: InCuBUS_reduced_lethality;
...
experiment fsRIDL type: batch repeat: 10 keep_seed: false until: nb_days >= (release_delay + 120) {
...
experiment InCuBUSfsRIDL type: batch repeat: 10 keep_seed: false until: nb_days >= (release_delay + 120) {
	parameter "InCuBUSfsRIDL" var:exp_InCuBUSfsRIDL init: true;
	parameter "Lethality" var:InCuBUS_lethality init: InCuBUS_reduced_lethality;
	parameter "Batch" var:is_batch init: true;
